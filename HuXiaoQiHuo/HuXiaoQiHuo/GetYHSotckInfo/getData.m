...
-(id)initWithUrl:(NSString*)url{
    if (self){
        ...
        if (lines.count>0) {
            ...
            // 加载完成执行此块
            NSError * error;
            [self Finished:[NSString stringWithContentsOfURL:[NSURL URLWithString:url] encoding:NSUTF8StringEncoding error:&error] url:url];
        }
	}
    return self;
}

- (void)Finished:(NSString *)content url:(NSString*)url
{
	self.status.text = @"";
...
-(void)changeData:(NSArray*)lines{
    ...
    for (idx = newArray.count-1; idx > 0; idx--) {
        ...
        dicItem[@"Date"] =[arr objectAtIndex:0];
        dicItem[@"Open"] =[arr objectAtIndex:1];
        dicItem[@"High"] =[arr objectAtIndex:2];
        dicItem[@"Low"] =[arr objectAtIndex:3];
        dicItem[@"Close"] =[arr objectAtIndex:4];
        dicItem[@"Volume"] =[arr objectAtIndex:5];
        dicItem[@"Adj"] =[arr objectAtIndex:6];
        CGFloat idxLocation = [lines indexOfObject:line];
        dicItem[@"MA5"] = [NSNumber numberWithFloat:[self sumArrayWithData:lines andRange:NSMakeRange(idxLocation, MA5)]];
        dicItem[@"MA10"] = [NSNumber numberWithFloat:[self sumArrayWithData:lines andRange:NSMakeRange(idxLocation, MA10)]];
        dicItem[@"MA20"] = [NSNumber numberWithFloat:[self sumArrayWithData:lines andRange:NSMakeRange(idxLocation, MA20)]];

        [data addObject:dicItem];
